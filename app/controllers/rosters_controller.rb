class RostersController < ApplicationAuthController
  ...
  def index
    [Date.today.prev_month, Date.today].each do |date|
      rosters.find_or_create_by(company_id: current_user.company_id, year: date.year, month: sprintf('%02d', date.month))
    end
    ...
    ActiveRecord::Base::transaction do
      ...
      (roster.date.beginning_of_month..roster.date.end_of_month).each do |date|
        roster.one_days.find_or_initialize_by(day: date) do |od|
          if od.new_record?
            if od.work_day?
              od.company = current_user.partner || current_user.company
              od.set_work!
            end
            od.save!
          end
  ...
  def create
    ...
    if @roster.save
      redirect_to [current_company, @roster], notice: 'Roster was successfully created.'
    else
      render :new
    end
  end
  ...
  def update
    if @roster.update_attributes(roster_params)
      redirect_to [current_company.to_company, @roster], notice: 'Roster was successfully updated.'
    else
      render :edit
    end
  end
  ...
  def destroy
    @roster.destroy
    redirect_to company_rosters_url(current_company), notice: 'Roster was successfully destroyed.'
  end
